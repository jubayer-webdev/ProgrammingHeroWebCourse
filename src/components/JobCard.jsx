...
const JobCard = ({ job }) => {
    const { _id, job_title, description, min_price, max_price, category, deadline, bid_count  } = job || {};
    ...
    return (
        <Link to={`/job/${_id}`} className="w-full max-w-sm px-4 py-3 bg-white rounded-md shadow-md hover:scale-[1.05] transition-all">
            <div className="flex items-center justify-between">
                <span className="text-xs font-light text-gray-800 ">Deadline: {new Date(deadline).toLocaleString()}</span>
            ...
            <div>
                ...
                <p className="mt-2 text-sm font-bold text-gray-600 ">Bid Count: {bid_count}</p>
            </div>
        </Link>
    );
};
